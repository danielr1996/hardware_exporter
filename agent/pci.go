...
func collectPCIDevices() []PCIDevice {
	...
	for _, e := range entries {
		...
		vendor := strings.TrimPrefix(readFirstLine(filepath.Join(base, "vendor")), "0x")
		device := strings.TrimPrefix(readFirstLine(filepath.Join(base, "device")), "0x")
		class := strings.TrimPrefix(readFirstLine(filepath.Join(base, "class")), "0x")
